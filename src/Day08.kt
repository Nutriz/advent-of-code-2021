...
fun main() {
    fun part1(input: String): Int {
        ...
        return output
            .flatten()
            .count {
                it.length < 5 || it.length == 7
            }
    }

    fun part2(input: String): Int {
        val formatted = input.lines()
            .map { line ->
                line.split(" | ")
                .map { it ->
                    it.split(" ")
        ...
        for (line in formatted) {
            val (patterns, output) = line

            val mapping = Array(10) { "" }
            mapping[1] = patterns.first { it.length == 2 }
            mapping[4] = patterns.first { it.length == 4 }
            mapping[7] = patterns.first { it.length == 3 }
            mapping[8] = patterns.first { it.length == 7 }
            // 3 is length 5 and overlaps 1
            mapping[3] = patterns.filter { it.length == 5 }.first { it.overlaps(mapping[1]) }
            mapping[9] = patterns.filter { it.length == 6 }.first { it.overlaps(mapping[3]) }
            mapping[0] = patterns.filter { it.length == 6 }.filter { it.overlaps(mapping[1]) && it.overlaps(mapping[7]) }
                .first { !it.overlaps(mapping[9]) }
            mapping[6] = patterns.filter { it.length == 6 }.first { !it.overlaps(mapping[0]) && !it.overlaps(mapping[9]) }
            mapping[5] = patterns.filter { it.length == 5 }.first { mapping[6].overlaps(it) }
            mapping[2] = patterns.filter { it.length == 5 }.first { !it.overlaps(mapping[3]) && !it.overlaps(mapping[5]) }

            result += output.map { mapping.indexOf(it) }.joinToString("").toInt()
        }
    ...
    check("part1", part1(testInput), 26)
    check("part2", part2(testInput), 61229)
